...
async def create(request:Request, entry: EntryCreateSchema, db: Session = Depends(get_db)):
...
async def update(request:Request, id: int, entry: EntryUpdateSchema, db: Session = Depends(get_db)):
