...
async def create_user(request:Request, user: UserCreateSchema, db:Session = Depends(get_db)):
...
async def updat_user(request:Request, id:int, user:UserUpdateSchema, db: Session = Depends(get_db)):
...
async def user_by_id_competition(request:Request, id:int, db:Session = Depends(get_db)):
